...
GERMAN_MARKERS = {
    "der", "die", "das", "eine", "einer", "eines", "einem", "einen",
    "ist", "nicht", "sind", "war", "waren", "wird", "werden", "wurde",
    "haben", "hat", "hatte", "hatten", "hast", "habt",
    "ich", "du", "sie", "wir", "ihr", "mich", "dich",
    "sich", "unser", "euer", "mein", "dein",
    "von", "zu", "auf", "aus", "bei", "nach", "vor", "über", "unter",
    "mit", "durch", "gegen", "ohne",
    "kann", "kannst", "muss", "müssen", "soll", "sollen", "will", "wollen",
    "aber", "oder", "denn", "weil", "wenn", "als", "ob", "dass",
    "auch", "noch", "schon", "immer", "wieder", "mehr", "sehr",
    "hier", "dort", "jetzt", "heute", "morgen", "gestern",
    "nein", "bitte", "danke", "entschuldigung",
    "gut", "schlecht", "groß", "klein", "schön",
    "Mann", "Frau", "Kind", "Haus", "Weg", "Tag", "Nacht", "Jahr", "Zeit",
    "Handy", "Auto", "Tür", "Fenster", "Tisch", "Stuhl", "Bett",
    "können", "dürfen", "möchten", "mögen",
    "geht", "mach", "komm", "geh", "bleib", "wart",
    "diese", "dieser", "dieses", "jene",
    "welche", "manche", "niemand",
    "etwas", "nichts", "alles", "jemand",
    "nur", "mal", "eben", "ruhig", "doch",
    "Beziehung", "Anwalt", "gewonnen", "belogen", "Fall",
    "beendete", "fing", "fangen", "willkommen", "Nachbarschaft",
    "Theoretisch", "Norden", "genau", "Kleinstadt", "kennen",
    "später", "früher", "werden", "wurden",
    "ja", "und", "nein",
    "für", "dem", "den", "des", "ein",
    "war", "habe", "hast", "haben", "hat",
    "warum", "wieso", "wie", "was", "wer", "wo", "woher", "wohin",
    "hier", "da", "dort",
    "jetzt", "gleich", "später",
    "wahr", "falsch", "richtig",
    "viel", "wenig",
    "solche", "welche",
    "damit", "darauf", "davon", "dazu", "daran", "dabei",
    "sonst", "zwar", "jedoch", "trotzdem",
    "oft", "selten", "manchmal", "stets",
    "gar", "sogar", "bereits",
    "gerne", "leider", "hoffentlich",
    "vielleicht", "wahrscheinlich", "sicher",
    "Stadt", "Land", "Welt", "Mensch", "Leben", "Liebe",
    "Wasser", "Brot", "Milch", "Wein", "Kaffee",
    "Hund", "Katze", "Vogel", "Fisch",
    "rot", "blau", "grün", "gelb", "schwarz", "weiß",
    "links", "rechts", "gerade",
    "oben", "unten",
    "Anfang", "Ende",
    "Hupe", "Bremsen", "Motor", "Reifen",
    "Stoff", "Farbe", "farb", "nachhaltig",
    "versuch", "Versuch",
    "meinem", "meiner", "deinem", "deiner", "seinem", "seiner",
    "Ordnung", "Ahnung", "Zustimmung", "vermacht", "Schraubstock",
    "Unbezahlbar", "eigentlich", "Seien", "unbesorgt",
    "keine", "volle", "schon", "Hallo", "Wunder", "wunder",
}
...
def has_german_patterns(text: str) -> bool:
    lower = text.lower()
    patterns = [
        r"\b(ord)nung\b", r"\bahnung\b", r"\bzustimm",
        r"\bvermacht\b", r"\bschraub", r"\bunbezahl",
        r"\bsei(en|st)\s+\w+\b", r"\bkeine\b", r"\bvolle\b",
        r"\bschaut\b", r"\btut\b", r"\bleid\b",
    ]
    import re
...
class SubtitleCorrector:
    ...
    def correct_line(self, line: str, reference: str | None = None, max_tokens: int = 200) -> str:
        ...
        text_out = re.sub(
            ...
            "", text_out, flags=re.DOTALL | re.IGNORECASE,
        ).strip()
    ...
    if input_file.suffix == ".srt":
        ...
        for i, block in enumerate(blocks):
            ...
            if len(lines) >= 3:
                ...
                corrected_text = corrector.correct_line(text_lines, reference=ref_text, max_tokens=max_tokens)
                ...
                corrected.append(block)
            if (i + 1) % 50 == 0:
                console.print(f"[dim]  Processed {i+1}/{total} blocks ({changed} changed, {skipped} skipped)[/dim]")
        result = "\n\n".join(corrected)
        console.print(f"[green]Done: {changed}/{total} corrected, {skipped} skipped[/green]")
    else:
        ref_text = None
        if ref_blocks:
            ref_text = "\n".join(ref_blocks[0].strip().split("\n")[2:]) if reference_file.suffix == ".srt" else reference_file.read_text().strip()
